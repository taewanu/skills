...
Install as a Claude Code plugin, from inside Claude Code:
...
To hack on the skills instead, clone the repo and run `scripts/link-skills.sh`. It symlinks every skill, `in-progress/` ones included, into `~/.claude/skills` and `~/.agents/skills`, so an edit here is live immediately and `git pull` is the whole update story.
