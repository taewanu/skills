...
- **Keep the split navigable with `data-see`.** A two-beat split can lose the thread — the reader sees "which signal?" but can't get to it. Add `data-see="9"` (accepts a list, e.g. `"9,40"`) to render a clickable **`↗ jump to L9`** chip in the note; clicking it jumps to the beat that focuses that line. Use it to wire the two halves both ways so the relationship is one click apart, no stretched connector needed.
