...
- **When two related lines are too far apart to share one screen, split them into two beats** — this is the settled pattern. Point at one, then the other, and let the notes carry the link ("…but why is the first key the oldest?" → "here's the contract it leans on"). Each end gets full-screen focus, which teaches better than cramming both into one view. Don't reach for a stretched connector here: if you do write a far `data-to` anyway, the page degrades gracefully — it keeps the primary range centered and draws an **off-screen edge marker** (`↑ L7`) toward the partner — but that marker is a *guardrail, not a target*. (Folding the gap or splitting the panel were considered and rejected: they drift toward the all-at-once reference model that's `visual-code-review`'s job, not this skill's.)
...
**Start from `templates/lecture.html`.** Copy the **mechanics verbatim** — the embedded **rough.js** bundle and the renderer that drives it (ellipse / box / line / hachure fill + draw-on animation), the scrollytelling layout (sticky code-col + scrolling `.note-step` cards the JS builds from `#beats`), scroll-driven activation, spotlight (dim/spot classes), the rough.js connector across the gutter, ◀ ▶ / dot / click-line jumps, theme/wrap, and the click-to-copy note + copy-transcript. Those are solved; re-deriving them adds bug risk. The **chrome** (mark color `--mark`, the mark opacity via `.sketch path { stroke-opacity }` so code reads through, the rough.js `pen` options like `roughness`/`bowing`, the note column width, the focus-band offset, which mark per beat, the floating tag) is yours to tweak.
...
3. **`#beats`** — one `<li class="beat">` per step. `data-lines` ("41" / "41-44" / "41,47"), `data-mark` (`circle` default / `box` / `underline` / `strike` / `highlight` / `none`), optional `data-to` for a connector, optional `data-tag` for a one-word floating punch. The `<li>` text is the spoken explanation (inline `<code>` allowed).
...
- **Click the note card** — copies the current step (with its `L—` ref) for pasting back.
